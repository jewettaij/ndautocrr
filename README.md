...
(or correlation time) is printed to the standard error.
  * By default, the correlation length is estimated by finding the location *j* where *C(j)/C(0)* crosses *1/e*.
...
by summing *Σ_j C(j)* from j=0 to j=L. (Graphing the autocorrelation function is always a good way to choose an appropriate "-L" parameter, especially if oscillations are present.)
  * If both are specified, then the threshold method is preferentially used *(unless C(j)/C(0) fails to decay enough before j reaches L).*
